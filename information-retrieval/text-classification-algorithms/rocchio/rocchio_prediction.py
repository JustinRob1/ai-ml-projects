...
def main():
    try:
        ...
        print("Usage: python rocchio_predict.py <model_file> <test_file>")
        return 1

    idf, centroids = read_model(model_file)
    C = centroids.keys()
    V = idf.keys()
    test_data = get_json_data(test_file)
    to_doc_vecs(test_data, idf)

    predictor = RocchioPredictor(centroids, V)
    ...
    with open(model_file, "r") as f:
        for line in f:
            if line.startswith("idf"):
                _, term, idf_weight = line.strip().split('\t')
                idf[term] = float(idf_weight)
            elif line.startswith("centroid"):
                _, c, centroid_vec_str = line.strip().split('\t')
                centroids[c] = DocumentVector(centroid_vec_str)
...
class RocchioPredictor:
    def __init__(self, centroids, V):
        '''
        centroids: dict of class names to DocumentVector
        '''
        ...
        for c in self.centroids:
            ...
            for term in d:
                if term not in self.V:
                    continue
                dist += (d[term] - self.centroids[c].get(term, 0)) ** 2
            dist = math.sqrt(dist)
            dists[c] = dist
