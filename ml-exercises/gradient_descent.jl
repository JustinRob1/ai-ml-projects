...
function epoch!(reg::SimpleStochasticRegressor, 
	...
	# Put each x, y pair into a tuple and push the tuple to the array pairs
	pairs = []
	for i in 1:length(X)
			pair = (X[i], Y[i])
			push!(pairs, pair)
	...
	# Randomize the tuples and put into the array random
	# First element of each tuple is X and the second is Y
	perm = randperm(length(pairs))
	random = pairs[perm]

	# Perform the formula 
	for i in 1:length(X)
			reg.w = reg.w - reg.η * (random[i][1] * reg.w - random[i][2]) * random[i][1]
	end
end
#### END SOLUTION
...
function epoch!(reg::MiniBatchRegressor,
	...
	# Calculate ceiling of n/N_batch
	CeilUpdates = ceil(length(X) / reg.b)
	println(CeilUpdates)
	# Calculate n/N_batch
	Updates = length(X) / reg.b
	println(updates)

	# If n the number of samples is divisible by the batch size then perform
	# the update equation as normal
	if CeilUpdates == Updates
		StartIndex = 1
		EndIndex = reg.b
		# Peform formula
		for i in 1:CeilUpdates
			if i != 1
				StartIndex += reg.b
				EndIndex += reg.b
			end
			...
			for j in StartIndex:EndIndex
				gt += (random[j][1] * reg.w - random[j][2]) * random[j][1]
			end
			gt *= (1 / reg.b)
			reg.w = reg.w - reg.η * gt
		end
	# If n the number of samples is not divisible by the batch size then the very
	# last batch should by normalized by the number of samples in the minibatch
	else
		StartIndex = 1
		EndIndex = reg.b
		# Execute formula
		for i in 1:CeilUpdates
			# If we are on the last batch the normalize by the number of samples in the minibatch
			if i == CeilUpdates
				Floor = floor(length(X) / reg.b) 
				...
				gt *= (1 / Floor)
				reg.w = reg.w - reg.η * gt
			# Else perform formula as normal
			else
				if i != 1
					StartIndex += reg.b
					EndIndex += reg.b
...
function epoch!(reg::MiniBatchHeuristicRegressor,
	...
	if CeilUpdates == Updates
		...
		for i in 1:CeilUpdates
			...
			# Heuristic for step step
			gt *= (1 / reg.b)
			η = 1/(1 + abs(gt))
			reg.w = reg.w - η * gt
		end
	# If n the number of samples is not divisible by the batch size then the very
	# last batch should by normalized by the number of samples in the mini back
	else
		StartIndex = 1
		EndIndex = reg.b
		# If we are on the last batch the normalize by the number of samples in the minibatch
		for i in 1:CeilUpdates
			if i == CeilUpdates
				...
				η = 1/(1 + abs(gt))
				reg.w = reg.w - η * gt
			# Else perform the formula as normal
			else
				if i != 1
					StartIndex += reg.b
					EndIndex += reg.b
...
let
	if __run_q3b_experiments
		...
		println("Mean (μ): ", mean(results3b["Mean"].test_error))
		println("Mean (σ): ", std(results3b["Mean"].test_error))
		println("Random (μ): ", mean(results3b["Random"].test_error))
		println("Random (σ): ", std(results3b["Random"].test_error))
		println("SimpleStochastic (μ): ", mean(results3b["SimpleStochastic"].test_error))
		println("SimpleStochastic (σ): ", std(results3b["SimpleStochastic"].test_error))
	end
end
...
let
	if __run_q3_experiments
		...
		println("Mean (μ): ", mean(results["Mean"].test_error))
		println("Mean (σ): ", std(results["Mean"].test_error))
		println("Batch (μ): ", mean(results["Batch"].test_error))
		println("Batch (σ): ", std(results["Batch"].test_error))
		println("Minibatch (μ): ", mean(results["Minibatch"].test_error))
		println("Minibatch (σ): ", std(results["Minibatch"].test_error))
		println("BatchHeuristic (μ): ", mean(results["BatchHeuristic"].test_error))
		println("BatchHeuristic (σ): ", std(results["BatchHeuristic"].test_error))
		println("MinibatchHeuristic (μ):", mean(results["MinibatchHeuristic"].test_error))
		println("MinibatchHeuristic (σ): ", std(results["MinibatchHeuristic"].test_error))
	end
end
