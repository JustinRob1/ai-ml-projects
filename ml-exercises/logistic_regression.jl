...
student = (name="Justin Robertson", email="jtrober1@ualberta.ca", ccid="jtrober", idnumber=1702786)
...
function loss(lm::AbstractModel, ce::CrossEntropy, X , Y)
	...
	b = size(X, 1)
    for i in 1:b
        σ = sigmoid(transpose(X[i]) * θ[i])
        loss += -Y[i] * log(σ) - (1 - Y[i]) * log(1 - σ)
    end
    return loss / b
	#### END SOLUTION
end;
...
function update!(lm::LinearModel, 
    ...
    lm.w .= lm.w .- (opt.η ./ sqrt.(opt.gbar .+ opt.ϵ)) .* g
	#### END SOLUTION	
end;
...
function epoch!(mbgd::MiniBatchGD, model::LinearModel, lossfunc, opt, X, Y)
	...
	#### BEGIN SOLUTION
	randn = shuffle(1:size(X, 1))
    for i = 1:mbgd.b:size(X, 1)
        index = randn[i:min(i+mbgd.b-1, size(X, 1))]
        X_batch = X[index, :]
        Y_batch = Y[index]
...
# H0: There is no significant difference between the misclassification error of logistic regression and polynomal logistic regression, μ1 = μ2
# Ha: There is a significant difference between the expected misclassification of logistic regression and polynomial logistic regression, μ1 > μ2
...
function tDistPValue(baseline_error::AbstractVector{Float64}, 		           learner_error::AbstractVector{Float64})
	...
	t = 0.0
	#### BEGIN SOLUTION
	diff = baseline_error - learner_error
    μ = mean(diff)
    σ = std(diff) / sqrt(m)
    t = μ / σ
	#### END SOLUTION
...
# With pvalue = 0.003897900662299114, the null hypothesis is rejected under a pvalueThreshold of 0.01. Therefore there is no significant difference between the errors of the logistic regression model and the polynomial logistic regression model under the null hypothesis. 
