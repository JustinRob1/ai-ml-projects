...
# ╔═╡ 6e070dde-de71-11eb-11e0-9161dfe3c5b6
# Import the packages and export their exported functions to the main namespace.
using StatsPlots, PlutoUI, Random

# ╔═╡ 0d9c0987-470d-4b2e-a446-b321a92e2b59
md"""
# Assignment 1 - Q4: Simulating Gaussian Distributions

Welcome to Pluto Notebooks! In this course you will be spending quite a bit of time working within these notebooks. These notebooks are an HTML/CSS/Javascript built interface for interacting with and working with Julia. They were inspiried by Jupyter, and you can learn more about the notebooks through [their github](https://github.com/fonsp/Pluto.jl). The notebooks are lightweight (with files that can be used by the normal julia interpretor), and track how notebook cells depend on each other to re-run cells when their dependencies change.

The only code in this notebook which is not found in julia's base is the plotting code provided by [StatsPlots](https://github.com/JuliaPlots/StatsPlots.jl) and [Plots](http://docs.juliaplots.org/latest/). Another package we use is [PlutoUI]() which contains utilities for building interfaces in pluto notebooks. To complete this assignment you will not need to import any other packages.
...
student = (name="Justin Robertson", email="jtrober1@ualberta.ca", ccid="jtrober1", idnumber=1702786)
...
mean(gd::GaussianDistribution) = gd.μ

# ╔═╡ d0d7c379-f03a-4dd9-a3f2-88835e2ecebb
stddev(gd::GaussianDistribution) = gd.σ

# ╔═╡ d281372b-1473-4e6c-8f9f-cb8f280d8f07
var(gd::GaussianDistribution) = gd.σ^2
...
# ╔═╡ cfe70deb-92bf-4c40-b691-4f7fb2254774
md"""
...
function mean(D::Vector{Float64})
	#### BEGIN SOLUTION
...
function var(D::Vector{Float64})
	...
	return total /= length(D) - 1
	end
	#### END SOLUTION
end

# ╔═╡ eb442864-b44d-4980-bb8f-912bc802d4d2
function stddev(D::Vector{Float64})
	#### BEGIN SOLUTION
	return sqrt(var(D))
	#### END SOLUTION
end
...
let
	...
	v = var(X)
	μ̂, σ̂, v
	if μ̂ == 3.0 && σ̂ == sqrt(2.0) && v == 2.0
		md"**Ok, but something is slightly wrong!** 🤔"
	elseif μ̂ == 3.0 && σ̂ == sqrt(2.5) && v == 2.5
		md"**Great job!** ✅🎉"
	else
		md"**Keep at it** ❌"
	end
...
let # example
	n = 3
	gd = GaussianDistribution(6.2, 0.1)
	dataset = sample(gd, n)
	for i in 1:n
		println(dataset[i])
	end	
...
# ╔═╡ b15ccdd1-2a1b-471f-95bf-a2bfdb237b7f
let # 4b
	n = 10
	gd = GaussianDistribution(0, 1.0)
	sample_arr = Float64[]
	for i in 1:5
		dataset = sample(gd, n)
		sample_mean = mean(dataset)
		print("M", i, ": ", sample_mean, " ")
		append!(sample_arr, sample_mean)
	end
	print("Sample Variance: ", var(sample_arr))
end
...
let # 4d, 4e
	n = 30
	gd = GaussianDistribution(0, 10.0)
	dataset = sample(gd, n)
	print("M: ", mean(dataset))
end
...
`plot_density` plots a histogram of the data `D` and a density estimated through a kernel density algorithm (see implementation on [github](https://github.com/JuliaPlots/StatsPlots.jl) for more details).

`plot_box_and_violin` plots a box plot over a violin plot. A violin plot shows the density of the sampled data (same as the `density` function), while the overlayed box plot shows the first quartile, median, and third quartile. More information can be found on [github](https://github.com/JuliaPlots/StatsPlots.jl) about these plotting utilites.

"""

# ╔═╡ 1e613f8c-4639-449e-8c40-f0be93c1ca46
function plot_density(D)
	histogram(
	...
	density!(D, color=:black, lw=2)
end

# ╔═╡ b62fa4e7-7ff6-46a0-8232-b106e8adbc20
function plot_box_and_violin(D)
	plt = violin(
...
Below are some sliders you can use to visualize different normal distribtuions interactively. The data is then plotted using the above plotting functions.

Mean: $(@bind mu Slider(-100.0 : 0.1 : 100.0; default=0.0, show_value=true))

Std Dev: $(@bind sigma Slider(0.1 : 0.1: 100.0; default=1.0, show_value=true))

number of samples: $(@bind n Slider(10 : 10 : 10000; default=100, show_value=true))

$(@bind resample_btn Button("Resample"))
"""

# ╔═╡ 6c23eb8c-bf97-462e-a0b0-5a72ff2ce1b7
gd = GaussianDistribution(mu, sigma)

# ╔═╡ aa4337e3-b4cc-406c-923a-fbb9927d91a5
gd.:μ
...
let
	...
	md"""
	**True mean:** $(gd.μ), **True Standard Deviation:** $(gd.σ)
	
	**Sample mean:** $(x̄), **Sample Standard Deviation:** $(σ̄)"""
end

# ╔═╡ e94d17ab-862e-4c13-9b65-a4df009c59a0
let
	plt_1 = plot_density(D)
	plt_2 = plot_box_and_violin(D)
	plot(plt_1, plt_2)
end
