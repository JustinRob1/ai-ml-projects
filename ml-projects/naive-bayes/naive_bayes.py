...
def train_nb(train_X, train_y, num_classes, **kwargs):
    ...
    for c in range(num_classes):
        class_indices = (train_y[:, 0] == c)
        class_data = train_X[class_indices, :]
        means[c, :] = np.mean(class_data, axis=0)
        covariances[c, :, :] = np.diag(np.var(class_data, axis=0))
        priors[c] = class_data.shape[0] / N
...
def predict_nb(params, X):
    ...
    for c in range(params.C):
        mean = params.means[c, :]
        covariance = params.covariances[c, :, :]
        prior = params.priors[c]
        likelihoods = -0.5 * np.sum(np.log(np.linalg.det(covariance))) \
                           - 0.5 * np.sum(np.multiply((X - mean), np.linalg.solve(covariance, (X - mean).T).T), axis=1) \
                           + np.log(prior)
        unnormalized_probs[:, c] = likelihoods
    max_likelihoods = np.max(unnormalized_probs, axis=1, keepdims=True)
    probs = np.exp(unnormalized_probs - max_likelihoods)
