...
class CBSState:
    ...
    def is_solution(self):
        ...
        """
        i = 0
        max_path_len = max(len(self._paths[path]) for path in self._paths)
        
        while i < max_path_len:
            states = set()
            for j in range(0, self._k):
                if i < len(self._paths[j]):
                    if self._paths[j][i] in states:
                        return False, (self._paths[j][i], i)
                    else:
                        states.add(self._paths[j][i])
            i += 1
        return True, None
    ...
    def successors(self):
        ...
        """
        sol, (conflict_state, conflict_time) = self.is_solution()
        if not sol:
            children_count = 0
            children = []
            for i in self._paths:
                if conflict_time < len(self._paths[i]) and self._paths[i][conflict_time] == conflict_state:
...
class CBS():
    def search(self, start):
        ...
        """
        start_cost, start_paths = start.compute_cost()
        if start_cost == (float('inf')):
            return start_paths, start_cost
        OPEN = []
