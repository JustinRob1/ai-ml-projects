...
class MRV(VarSelector):
    ...
    def select_variable(self, grid):
        ...
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                if len(grid.get_cells()[i][j]) > 1 and len(grid.get_cells()[i][j]) < min_domain:
                    min_domain = len(grid.get_cells()[i][j])
                    min_var = (i, j)
        return min_var
...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        while Q:
            ...
            rows_assigned, row_failure = self.remove_domain_row(grid, var[0], var[1])
            columns_assigned, column_failure = self.remove_domain_column(grid, var[0], var[1])
            units_assigned, unit_failure = self.remove_domain_unit(grid, var[0], var[1])
            
            if row_failure or column_failure or unit_failure:
                return False
            
            if rows_assigned:
                for var in rows_assigned:
                    Q.append(var)
            if columns_assigned:
                for var in columns_assigned:
                    Q.append(var)
            if units_assigned:
                for var in units_assigned:
                    Q.append(var)
    ...
    def search(self, grid, var_selector):
        ...
        for d in grid.get_cells()[var[0]][var[1]]:
            if grid.is_value_consistent(d, var[0], var[1]):
                ...
                ri = ac3.consistency(grid_copy, [var])
                if ri:
